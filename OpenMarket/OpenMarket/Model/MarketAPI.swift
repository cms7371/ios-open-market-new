...
struct MarketAPI {
    ...
    static func getProducts(pageNumber: Int, itemCount: Int = 20, completionHandler: @escaping (Data) -> Void) {
        HTTPManager.getData(url: baseURL, path: basePath, query: ["page_no": pageNumber, "items_per_page": itemCount],
                        completionHandler: completionHandler)
    }

    static func getProduct(id: Int, completionUIHandler: @escaping (Data) -> Void) {
        HTTPManager.getData(url: baseURL, path: basePath + ["\(id)"],
                            query: nil, completionHandler: completionUIHandler)
    }
}
